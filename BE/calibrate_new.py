    ...
    if len(required_pts) >= NUM_REQUIRED:
        ...
        if _H is not None:
            try:
                ...
                # Sidelines (full half-court depth)
                _proj_line(0.0, 0.0, 0.0,       half_y, C_EXPECTED, 2)
                _proj_line(court_w, 0.0, court_w, half_y, C_EXPECTED, 2)
                # Half-court line
                _proj_line(0.0, half_y, court_w, half_y, (80, 200, 80), 1)
...
def _draw_reprojection_preview(img_base: np.ndarray, H: np.ndarray) -> np.ndarray:
    ...
    for gx in range(0, int(court_w) + 200, 200):
        cv2.line(preview, proj(float(gx), 0), proj(float(gx), half_y), (50,50,50), 1)

    # Sidelines + baseline
    for cx in [0.0, court_w]:
        cv2.line(preview, proj(cx, 0), proj(cx, half_y), (0,255,255), 2)
    ...
    hc_l = proj(0.0, half_y);  hc_r = proj(court_w, half_y)
